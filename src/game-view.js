...
function notesMarkup(notes, size) {
  ...
  return `<span class="notes-grid" style="--note-cols:${Math.ceil(Math.sqrt(size))}">${Array.from({ length: size }, (_, index) => `<i class="note-slot">${notes.includes(index + 1) ? (runtime.save.theme === "shapes" ? shapeSvg(index + 1, true) : index + 1) : ""}</i>`).join("")}</span>`;
}
